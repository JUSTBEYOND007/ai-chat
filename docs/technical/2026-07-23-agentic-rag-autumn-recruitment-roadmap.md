...
- [ ] 避免空白、过短和高度重复 chunk。
- [ ] 记录 chunk 字符数、估算 Token、内容 Hash 和文档版本。
- [ ] 在 baseline 中单独记录由切片质量造成的失败样本，不盲目同时调整所有参数。

### 4.3 Query Rewrite

- [ ] 对“它还有哪些优点”等依赖历史的问题生成独立检索 query。
- [ ] Rewrite 输入使用当前问题、预算内最近历史和 Summary Memory，不直接拼接全部对话。
- [ ] 保存 `originalQuery`、`rewrittenQuery`、耗时和是否回退。
- [ ] Query Rewrite 超时、空结果或异常时使用原始问题回退。
- [ ] 校验改写结果没有改变知识库、实体和用户原始意图。
- [ ] 普通完整问题允许跳过 Rewrite，避免无意义的额外模型调用。

### 4.4 PostgreSQL 双路召回

- [ ] 保留 pgvector 语义召回，并记录原始向量分数和排名。
- [ ] 为 chunk 增加 PostgreSQL `tsvector` 字段和 GIN 索引。
- [ ] 增加全文/关键词召回，覆盖专有名词、版本号、错误码和精确短语。
- [ ] 使用参数化 SQL，并继续限制到当前用户拥有的知识库。
- [ ] 向量和关键词通道分别配置候选数量，不直接把全部结果送入模型。
- [ ] 任一路召回失败时允许另一通道独立回退。

### 4.5 融合、阈值与结果多样性

- [ ] 使用 RRF 合并向量排名和关键词排名；明确 RRF 是排名融合，不冒充模型 reranker。
- [ ] 保留每个候选的 vector rank、keyword rank 和 fused score。
- [ ] 根据评测集校准最低相关阈值，不凭主观设置固定魔法数字。
- [ ] 无候选通过阈值时返回 `NO_RELIABLE_CONTEXT`，由 Agent 明确拒答或追问。
- [ ] 对同文档相邻 chunk、文本高度重合 chunk 做去重。
- [ ] 限制单篇文档进入最终上下文的片段数量，提升来源多样性。
...
- [ ] 为 RAG 片段分配独立 Token Budget。
- [ ] 综合相关度、来源多样性和 Token 成本选择最终片段。
- [ ] 保存最终采用和被过滤的候选及原因。
- [ ] 回答引用与最终送入模型的片段严格一致。
...
下一步立即从第六轮开始。暂不先做缓存、BullMQ、模型 reranker 或记忆控制，避免在没有检索基线时分散主线。
